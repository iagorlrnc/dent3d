...
function getSpecialistImage(specialty: string): string {
  const norm = specialty.toLowerCase()
  if (norm.includes("implanto")) {
    return "https://images.unsplash.com/photo-1559839734-2b71ea197ec2?w=400&fit=crop&q=80"
  }
  if (
    norm.includes("orto") ||
    norm.includes("aparelho") ||
    norm.includes("alinhador")
  ) {
    return "https://images.unsplash.com/photo-1622253692010-333f2da6031d?w=400&fit=crop&q=80"
  }
  if (
    norm.includes("estét") ||
    norm.includes("estet") ||
    norm.includes("lente") ||
    norm.includes("faceta")
  ) {
    return "https://images.unsplash.com/photo-1594824813573-246434e33963?w=400&fit=crop&q=80"
  }
  if (norm.includes("period") || norm.includes("gengiva")) {
    return "https://images.unsplash.com/photo-1537368910025-700350fe46c7?w=400&fit=crop&q=80"
  }
  return "https://images.unsplash.com/photo-1559839734-2b71ea197ec2?w=400&fit=crop&q=80"
}
...
export function Team() {
  ...
  useEffect(() => {
    async function fetchSpecialists() {
      try {
        const { data } = await specialistQueries.listActive()
        setSpecialists(data ?? [])
      } catch (error) {
        console.error(error)
      } finally {
  ...
  if (specialists.length === 0) {
    return (
      <section
        id="equipe"
        className="py-24 px-6 md:px-20 bg-gradient-to-b from-cream to-ivory relative"
      >
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="text-center mb-20">
            ...
            <p className="text-stone-muted font-light mt-4 max-w-lg mx-auto text-sm leading-relaxed">
              Profissionais dedicados, com formação de excelência internacional e
              o compromisso de cuidar da sua saúde com máxima precisão.
            </p>
          </div>
          ...
          <div className="max-w-md mx-auto text-center p-12 bg-white/40 border border-white/70 rounded-[32px] backdrop-blur-md shadow-sm">
            ...
            <p className="text-dark font-light text-base mb-2">
              Nenhum especialista disponível
            </p>
            <p className="text-stone-muted text-xs font-light leading-relaxed">
              Estamos atualizando nossa equipe. Assim que houver especialistas cadastrados no painel administrativo, eles aparecerão aqui.
            </p>
          </div>
        </div>
      </section>
          ...
          {specialists.map((member) => (
            <div key={member.id} className="text-center group">
              <div className="w-full aspect-[3/4] rounded-[32px] overflow-hidden mb-6 relative bg-ivory shadow-[0_10px_30px_rgba(74,123,111,0.02)] border border-white/60 hover:border-gold/30 hover:shadow-lg transition-all duration-500">
                <img
                  src={member.image_url || getSpecialistImage(member.specialty)}
                  alt={member.name}
                ...
                <div className="absolute bottom-5 left-5 bg-white/80 backdrop-blur-md border border-gold/20 px-3.5 py-1.5 rounded-2xl opacity-0 group-hover:opacity-100 transition-all duration-500 translate-y-3 group-hover:translate-y-0">
                  <span className="text-[9px] tracking-wider uppercase text-stone-muted font-sans font-semibold">
                    {member.cro || "Registro profissional"}
                  </span>
                </div>
              </div>
